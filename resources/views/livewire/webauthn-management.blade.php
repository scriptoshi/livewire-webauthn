<section class="mt-5 space-y-6 w-full">
    @if ($message)
        <div
            class="p-4 mb-4 rounded {{ $messageType === 'success' ? 'bg-green-50 text-green-800 dark:bg-green-900 dark:text-green-100' : 'bg-red-50 text-red-800 dark:bg-red-900 dark:text-red-100' }}">
            {{ $message }}
        </div>
    @endif
        ...
        <div class="bg-white dark:bg-gray-850">
            ...
            <div class="mt-4">
                <flux:input wire:model="newCredentialName"
                    placeholder="{{ __('Security Key Name (e.g. Yubikey, Phone)') }}" class="w-full mb-4" />
            </div>
    ...
    @elseif (!$enabled || count($credentials) === 0)
        <div class="bg-white dark:bg-gray-850">
            ...
            <div class="mt-5 flex items-center justify-end gap-4 space-x-4">
                @error('credentials')
                    <div class="mt-2 text-sm text-red-600 dark:text-red-400">
                        {{ $message }}
                    </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500 dark:text-gray-400">
                                            {{ __('Last used') }}:
                                            {{ $credential->last_used_at ? $credential->last_used_at->diffForHumans() : __('Never') }}
                                        </div>
                                    </div>
                                </div>
